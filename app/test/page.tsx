...
function TestQuestion() {
  ...
  const handleNext = useCallback(async () => {
    ...
    setCurrentIdx((i) => i + 1);
  }, [currentAnswer, isLastQuestion, tieBreakerMode, answers, submitTest, sessionId, router]);
    ...
    return (
      <div className="flex min-h-screen items-center justify-center bg-gradient-to-b from-[#0CB6FF] via-[#1ABEFF] to-[#2BD3FF] p-4">
        <div className="w-full max-w-7xl rounded-3xl bg-white/95 p-8 text-center shadow-2xl">
          ...
          <h2 className="font-dynapuff text-2xl font-bold text-ocean-deep">You already took the test!</h2>
          ...
          {RATING_LABELS.map((label, idx) => {
            ...
            return (
              <button key={score} onClick={() => handleAnswerAndAdvance(score)}
                className={`w-full rounded-full border-[2.5px] border-[#0A3D62]/80 bg-white px-6 py-3.5 text-center font-dynapuff text-sm font-semibold text-[#0A3D62] transition-all active:translate-y-[2px] active:shadow-none ${currentAnswer === score ? "bg-[#0A3D62] text-white shadow-none translate-y-[2px]" : "shadow-[0_4px_0_#0A3D62]/30 hover:shadow-[0_5px_0_#0A3D62]/40 hover:translate-y-[-1px]"}`}>
                {label}
              </button>
            );
          })}
